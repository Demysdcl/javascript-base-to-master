<script lang="ts">
  ...
  import { secondsToTime } from '../../utils/timer-utils'
...
<div class="config-modal" class:show>
  <form class="modal-content" on:submit|preventDefault={handleSave}>
    ...
    <LabelInput
      title={`Pomodoro time (${secondsToTime(editable.pomodoroTime)} min)`}
      bind:value={editable.pomodoroTime} />

    <LabelInput
      title={`Short rest time (${secondsToTime(editable.shortRestTime)} min)`}
      bind:value={editable.shortRestTime} />

    <LabelInput
      title={`Long rest time (${secondsToTime(editable.longRestTime)} min)`}
      bind:value={editable.longRestTime} />
